...
def get_best_nest(nest,newnest,fitness,n,dim,objf,trainInput,trainOutput,testInput, testOutput):
# Evaluating all new solutions
    tempnest=numpy.zeros((n,dim))
    tempnest=numpy.copy(nest)
    for j in range(0,n):
    #for j=1:size(nest,1),
        fnew=objf(newnest[j,:],trainInput,trainOutput,dim,testInput, testOutput);
...
def CS(objf,lb,ub,dim,n,N_IterTotal,trainInput,trainOutput,testInput, testOutput):
...
#    Ub=[ub]*nd
    ...
    fmin,bestnest,nest,fitness =get_best_nest(nest,new_nest,fitness,n,dim,objf,trainInput,trainOutput,testInput, testOutput)
    # Main loop counter
    for iter in range (0,N_IterTotal):
        ...
        fnew,best,nest,fitness=get_best_nest(nest,new_nest,fitness,n,dim,objf,trainInput,trainOutput,testInput, testOutput)
