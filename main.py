...
ExportToFile2="solutions"+time.strftime("%Y-%m-%d-%H-%M-%S")+".csv"
            ...
            for k in range (0,NumOfRuns):
                ...
                if(Export==True):
                    ...
                    with open(ExportToFile2, 'a',newline='\n') as out2:
                        ...
                        #if (Flag==False): # just one time to write the header of the CSV file
                            #header= numpy.concatenate([["Optimizer","Dataset","Experiment"],CnvgHeader1,CnvgHeader1])
                            #  writer.writerow(header)
                        a=numpy.concatenate([[x.best],x.bestIndividual])
                        writer.writerow(a)
                        out2.close()
                    #####-----------------------------------
                    #------------------------------------------------------------------------------------------
