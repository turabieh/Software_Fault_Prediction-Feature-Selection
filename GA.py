...
def mutation(offspring, chromosomeLength, lb, ub):
    ...
    mutationValue=numpy.random.randint(0,2)   # 0 or 1
    offspring[mutationIndex] = mutationValue


def GA(objf,lb,ub,dim,PopSize,iters,trainInput,trainOutput,testInput, testOutput):
    ...
    #ga=numpy.random.uniform(0,1,(PopSize,dim)) *(ub-lb)+lb # continous
    ga=numpy.random.randint(2, size=(PopSize,dim)) # binary
    ...
    for l in range(iters):
        ...
        for i in range(0,PopSize):
            ...
            #ga[i,:]=numpy.clip(ga[i,:], lb, ub) no need for bunary solutions
            while numpy.sum(ga[i,:])==0:   
                ga[i,:]=numpy.random.randint(2, size=(1,dim))

            # Calculate objective function for each search agent
            fitness=objf(ga[i,:],trainInput,trainOutput,dim,testInput, testOutput) 
